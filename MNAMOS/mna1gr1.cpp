...
Transistor MOS:M<nome> <nód> <nóg> <nós> <nób> <NMOS ou PMOS> L=<comprimento> W=<largura> <K> <Vt0> <lambda> <gamma> <fi> <Ld>
...
/* Foram colocados limites nos formatos de leitura para alguma protecao
  ...
  *p,
  ...
  mostraEstampa,mostraNP,mostraMOS,mostraEstampaAC, iniciaConduzindo, modoMonstro,
  temAc=0;
FILE *arquivo;
      ...
      if (netlist[i].npmos[0]=='N'){
        ...
        if (VGS>(-VT)) {
         ...
         cbg=((2*netlist[i].k/0.02)*netlist[i].comprimento*netlist[i].largura);
            ...
            g=((netlist[i].k*netlist[i].largura/netlist[i].comprimento)*(2*(-VGS-VT)*(-VDS)-(VDS*VDS))*(1+netlist[i].lambda*(-VDS))) +gm*VGS +gmb*vbsAux +gds*VDS;
            id=((netlist[i].k*netlist[i].largura/netlist[i].comprimento)*(2*(-VGS-VT)*(-VDS)-(VDS*VDS))*(1+netlist[i].lambda*(-VDS)));
            cgs=cgd=(((2*netlist[i].k/0.02)*netlist[i].largura*netlist[i].ld)+((netlist[i].k/0.02)*netlist[i].comprimento*netlist[i].largura));
            cbg=0;
        }
        else {
            netlist[i].y=2;
...
int InicializarArquivos(char temAc)
{
        ...
        fclose(arquivo);
        if (modoMonstro && tentativas>1) { PlaySound(TEXT("QUENAOVAI"),NULL, SND_RESOURCE | SND_ASYNC); Sleep(2000); }
        if (modoMonstro) PlaySound(TEXT("BIRL"),NULL, SND_RESOURCE | SND_ASYNC);
        printf("\nPonto de Operacao:\n");
        strcpy(txt,"Tensao");
        for (i=1; i<=nv; i++) {
...
int ResolverNewtonRaphson(void)
{
  ...
  for (tentativas=0; tentativas<=MAX_RANDOM && fabs(maiorErro)>TOLERANCIA; tentativas++) {
    ...
    if (modoMonstro && tentativas && !(tentativas%5)){ PlaySound(TEXT("EITA"),NULL, SND_RESOURCE | SND_ASYNC); Sleep(2200); }
    if (tentativas) printf("Reiniciando variaveis...\n");
    if (modoMonstro && tentativas && (tentativas%2) && (tentativas%5)) { PlaySound(TEXT("NAOVAI"),NULL, SND_RESOURCE | SND_ASYNC); Sleep(600); }
    if (modoMonstro && tentativas && !(tentativas%2) && (tentativas%5)){ PlaySound(TEXT("URL"),NULL, SND_RESOURCE | SND_ASYNC); Sleep(600); }
...
void __fastcall TForm1::Abrir1Click(TObject *Sender)
{
 ...
 if (modoMonstro) PlaySound(TEXT("SHOW"),NULL, SND_RESOURCE | SND_ASYNC);
 denovo:
  /* Leitura do netlist */
  ne=0; nv=0; strcpy(lista[0],"0");
  if (!OpenDialog1->Execute()) {
   PlaySound(TEXT("NEGATIVA"),NULL, SND_RESOURCE | SND_ASYNC);
    return;
  }
  ...
  while (fgets(txt,MAX_LINHA,arquivo)) {
    ...
    if (tipo=='R' | tipo=='C' | tipo=='L') {
      ...
      if(Numero(na,&netlist[ne].a))  {
		PlaySound(TEXT("NEGATIVA"),NULL, SND_RESOURCE | SND_ASYNC);
		return; 
		}
      if(Numero(nb,&netlist[ne].b))   {
		PlaySound(TEXT("NEGATIVA"),NULL, SND_RESOURCE | SND_ASYNC);
		return; 
		}
    }
      ...
      if(Numero(nc,&netlist[ne].c)) {
		PlaySound(TEXT("NEGATIVA"),NULL, SND_RESOURCE | SND_ASYNC);
		return; 
		}
      if(Numero(nd,&netlist[ne].d))  {
		PlaySound(TEXT("NEGATIVA"),NULL, SND_RESOURCE | SND_ASYNC);
		return; 
		}
    }
      ...
      if (LocalizaIndutores(indutorA,&netlist[ne].a)) {
		PlaySound(TEXT("NEGATIVA"),NULL, SND_RESOURCE | SND_ASYNC);
		return; 
		} /* Aproveita que a variavel nao e usada pra salvar a posicao do indutor A  no netlist */
      if (LocalizaIndutores(indutorB,&netlist[ne].b)) {
		PlaySound(TEXT("NEGATIVA"),NULL, SND_RESOURCE | SND_ASYNC);
		return; 
		}
  ...
  if(!temAc) {
    printf("Linha .AC com especificacoes da frequencia nao foi detectada.\nSerao utilizadas as frequencias do menu avançado\n");
    temAc=!temAc;
  }
  /* Resolve o sistema */
  if (!temTransistor) {
        ...
        if (InicializarArquivos(temAc)){
          PlaySound(TEXT("NEGATIVA"),NULL, SND_RESOURCE | SND_FILENAME);
          return;
        }
        if (npontos-1){
            if (escala[0]=='L') {
  ...
  Sleep(600);
  if (modoMonstro) PlaySound(TEXT("MAISUM"),NULL, SND_RESOURCE | SND_ASYNC);
  return;
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TForm1::Mtodo1Click(TObject *Sender)
{
  PlaySound(TEXT("TRAPEZIO"),NULL, SND_RESOURCE | SND_ASYNC);
}
//---------------------------------------------------------------------------
void __fastcall TForm1::Sobre1Click(TObject *Sender)
{
  PlaySound(TEXT("A37"),NULL, SND_RESOURCE | SND_ASYNC);
  Sleep(2000);
  PlaySound(TEXT("AQUI"),NULL, SND_RESOURCE | SND_ASYNC);
}
//---------------------------------------------------------------------------
void __fastcall TForm1::N131Click(TObject *Sender)
{
  PlaySound(TEXT("A13"),NULL, SND_RESOURCE | SND_ASYNC);
}
//---------------------------------------------------------------------------
void __fastcall TForm1::Ajuda1Click(TObject *Sender)
{
  PlaySound(TEXT("AJUDA"),NULL, SND_RESOURCE | SND_ASYNC);
}
//---------------------------------------------------------------------------
void __fastcall TForm1::N1Click(TObject *Sender)
{
  N1->Checked=!N1->Checked;
  modoMonstro=N1->Checked;
  PlaySound(TEXT("JAULA"),NULL, SND_RESOURCE | SND_ASYNC);
}
//---------------------------------------------------------------------------
